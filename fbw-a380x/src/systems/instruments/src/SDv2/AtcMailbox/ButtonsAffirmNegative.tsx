...
export interface ButtonsAffirmNegativeProps extends ComponentProps {
...
export class ButtonsAffirmNegative extends DisplayComponent<ButtonsAffirmNegativeProps> {
  ...
  private readonly isAffirmVisible = Subject.create<boolean>(false);
  private readonly isNegativeVisible = Subject.create<boolean>(false);
  ...
  private updateButtonVisibility = (): void => {
    ...
    let isAffirmVisible = false;
    let isNegativeVisible = false;
    ...
    this.isAffirmVisible.set(isAffirmVisible);
    this.isNegativeVisible.set(isNegativeVisible);
    ...
    return (
      <>
        <Button
          label="AFFIRM"
          onClick={() => {
            this.props.setMessageStatus(this.props.message.get().UniqueMessageID, 4);
          ...
          containerStyle={this.isAffirmVisible.map((visible) => (visible ? '' : 'display: none;'))}
          disabled={this.buttonsBlocked}
        ></Button>
        <Button
          label="NEGATIVE"
          onClick={() => this.props.setMessageStatus(this.props.message.get().UniqueMessageID, 5)}
          buttonStyle="height: 50px; justify-content: flex-end;"
          containerStyle={this.isNegativeVisible.map((visible) => (visible ? '' : 'display: none;'))}
          disabled={this.buttonsBlocked}
        ></Button>
        <Button
          label="SEND"
